...
const tr = (text, status) => ({ text, status });

async function main() {
  const count = await prisma.product.count();
  if (count > 0) {
    console.log(`[seed] ${count} products already present — skipping seed.`);
    return;
  }
  ...
  await prisma.project.create({
    data: { id: "ember-main", productId: "ember", name: "Ember Saga — 본편" },
  });

  await prisma.speaker.createMany({
    data: [
      { id: "kaela", productId: "ember", name: "Kaela", role: "관문 수비대장", persona: "24세 · 직설적이고 경계심이 강함", tones: { ko: { tone: "반말 · 단호", register: "반말", sampleLine: "넌 날 못 지나가." }, ja: { tone: "ぞんざい · 断固", register: "タメ口", sampleLine: "あんたは通さない。" }, "zh-CN": { tone: "直接 · 强硬", register: "随意", sampleLine: "你过不去。" } } },
      { id: "merchant", productId: "ember", name: "Old Merchant", role: "떠돌이 상인", persona: "노년 · 능청스럽고 흥정에 능함", tones: { ko: { tone: "하게체 · 능청", register: "하게체", sampleLine: "흥정 한번 해보겠나, 나그네?" }, ja: { tone: "老獪 · 軽妙", register: "丁寧", sampleLine: "ひとつ取引でもどうかね、旅人さん？" }, "zh-CN": { tone: "圆滑 · 诙谐", register: "客气", sampleLine: "做笔买卖如何，旅人？" } } },
      { id: "lyra", productId: "ember", name: "Lyra", role: "동료 마법사", persona: "20세 · 다정하지만 단호한 면이 있음", tones: { ko: { tone: "존댓말 · 차분", register: "존댓말", sampleLine: "조금만 더 버텨요. 길이 보여요." }, ja: { tone: "丁寧 · 穏やか", register: "丁寧", sampleLine: "もう少しだけ。道が見えます。" }, "zh-CN": { tone: "礼貌 · 沉稳", register: "礼貌", sampleLine: "再坚持一下，我看到路了。" } } },
      { id: "narration", productId: "ember", name: "Narration", role: "내레이션", persona: "3인칭 서술 · 문어체", tones: { ko: { tone: "문어체 · 묘사", register: "나레이션" }, ja: { tone: "文語 · 描写", register: "ナレーション" }, "zh-CN": { tone: "书面 · 描写", register: "旁白" } } },
    ],
  });
  ...
  await prisma.contextNote.createMany({
    data: [
      { id: "c1", productId: "ember", scene: "act II · sc.3 — 관문", arc: "공허 진입", note: "일행이 공허로 들어가는 유일한 관문 앞. 카엘라가 길을 막아선다. 긴장감 있고 짧은 대사 위주.", guides: { ko: "짧고 단호하게. 카엘라는 반말.", ja: "短く断固として。", "zh-CN": "简短强硬。" } },
      { id: "c2", productId: "ember", scene: "act II · sc.4 — 시장", arc: "공허 진입", note: "공허 변두리 시장. 음울하지만 활기. 상인의 능청스러운 호객.", guides: { ko: "능청스럽고 구어적으로.", ja: "軽妙な口調で。", "zh-CN": "诙谐口语化。" } },
    ],
  });
